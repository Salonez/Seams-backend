...
def test_invalid_user(clear_and_register):
    resp = requests.get(config.url + 'dm/details/v1', params={'token':'dodytoken', 'dm_id': pytest.dm1.json()['dm_id']})
    assert (resp.status_code == 403)

def test_invalid_dm_id(clear_and_register):
    resp = requests.get(config.url + 'dm/details/v1', params={'token':pytest.creator.json()['token'], 'dm_id': 34567})
    assert (resp.status_code == 400)

def test_not_a_member(clear_and_register):
    resp = requests.get(config.url + 'dm/details/v1', params={'token':pytest.user3.json()['token'], 'dm_id': pytest.dm2.json()['dm_id']}) 
    assert (resp.status_code == 403)
    
def test_details_working(clear_and_register):
    resp = requests.get(config.url + 'dm/details/v1', params={'token':pytest.creator.json()['token'], 'dm_id': pytest.dm1.json()['dm_id']})
    resp_data = resp.json()
    print(resp_data)
    assert(resp.status_code == 200)
    assert(resp_data.keys() == {'name', 'members'})
    assert isinstance(resp_data['name'], str)
    assert(len(resp_data['members']) == 4)