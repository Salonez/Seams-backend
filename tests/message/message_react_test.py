...
def test_base_case_channel(clear_and_register):
    ...
    resp = requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 1})
    assert resp.json() == {}
    assert resp.status_code == 200

def test_base_case_dm(clear_and_register):
...
def test_incorrec_user(clear_and_register):
    message =  requests.post(config.url + 'message/send/v1', json={'token':pytest.user1.json()['token'],'channel_id':pytest.channel1.json()['channel_id'],'message':"Test"})
    resp = requests.post(config.url + 'message/react/v1', json={'token': pytest.user2.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 1})
...
def test_incorrect_message_id(clear_and_register):
    ...
    resp = requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': 0, 'react_id': 1})
    assert resp.status_code == InputError.code

def test_incorrect_react_id(clear_and_register):
    message =  requests.post(config.url + 'message/send/v1', json={'token':pytest.user1.json()['token'],'channel_id':pytest.channel1.json()['channel_id'],'message':"Test"})
    resp = requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 0})
    assert resp.status_code == InputError.code

def test_duplicate_react(clear_and_register):
    message =  requests.post(config.url + 'message/send/v1', json={'token':pytest.user1.json()['token'],'channel_id':pytest.channel1.json()['channel_id'],'message':"Test"})
    requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 1})
    resp = requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 1})
    assert resp.status_code == InputError.code

def test_non_duplicate_react(clear_and_register):
    requests.post(config.url + 'channel/join/v2', json={'token': pytest.user2.json()['token'], 'channel_id': pytest.channel1.json()['channel_id']})
    message =  requests.post(config.url + 'message/send/v1', json={'token':pytest.user1.json()['token'],'channel_id':pytest.channel1.json()['channel_id'],'message':"Test"})
    message2 =  requests.post(config.url + 'message/send/v1', json={'token':pytest.user1.json()['token'],'channel_id':pytest.channel1.json()['channel_id'],'message':"Test"})
    requests.post(config.url + 'message/react/v1', json={'token': pytest.user1.json()['token'], 'message_id': message.json()['message_id'], 'react_id': 1})
