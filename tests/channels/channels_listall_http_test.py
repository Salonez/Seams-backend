...
def test_access_error_http():
    response = requests.get(url + '/channels/list/v2', json={'token': 'hello'})
    assert response.status_code == 403

def test_two_channel(clear_and_register):
    response = requests.get(url + '/channels/listall/v2', params={'token': pytest.user1.json()['token']})
    response_data = response.json()
    assert (response.status_code == 200)
    assert(len(response_data['channels']) == 2)

def test_channels_list__all_v2_output_type_http(clear_and_register):
    response = requests.get(url + "channels/listall/v2", params={'token': pytest.user2.json()['token']})
    response_data = response.json()
